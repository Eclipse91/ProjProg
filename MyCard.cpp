...
void MyCard::NewSublist(wxCommandEvent &evt) {
    if(nameSublist->GetValue().empty()){
        wxMessageDialog *dial = new wxMessageDialog(NULL,
                                                    wxT("Write a sublist's name"), wxT("Exclamation"),
...
void MyCard::NewSublist() {
    if(!NewSublistCounter){
        for(int i=0;i<numberSublist*2;i++){
            if(i%2==0){
                stringSublist[i/2]=*AttemptLabelSublist[i];
                staticSublist[i/2]->SetLabel(stringSublist[i/2]);
            }else{
                if(*AttemptLabelSublist[i]=="0"){
                    labelsSublist[(i+1)/2-1]=false;
                    checkboxSublist[(i+1)/2-1]->SetValue(labelsSublist[(i+1)/2-1]);
                }else if(*AttemptLabelSublist[i]=="1"){
                    labelsSublist[(i+1)/2-1]=true;
                    checkboxSublist[(i+1)/2-1]->SetValue(labelsSublist[(i+1)/2-1]);
                }
            }
        }
        coverPanel->SetPosition(wxPoint(10,40+numberSublist*25));
        coverPanel->SetSize(wxSize(220,400-numberSublist*25));
        NewSublistCounter=NewSublistCounter+1;
...
void MyCard::NewMember(wxCommandEvent &evt) {
    if(nameMember->GetValue().empty()){
        wxMessageDialog *dial = new wxMessageDialog(NULL,
                                                    wxT("Write a member's name"), wxT("Exclamation"),
                                                    wxOK | wxICON_EXCLAMATION);
        dial->ShowModal();
    }else{
        if(numberMember==0){
...
void MyCard::NewMember() {
    if(!NewMemberCounter){
        for(int i=0;i<numberMember;i++){
            labelMember[i]=*AttemptLabelMember[i];
            Member[i]->SetLabel(labelMember[i]);
        }

        hidingPanel->SetPosition(wxPoint(10,140+(numberMember*25)));
        hidingPanel->SetSize(wxSize(130,85-(numberMember*25)));
